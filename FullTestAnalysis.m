...
epochs = 5;
config_data_file = dir(['OUTPUT\',Tests(2).name,'\*as_run.ini']);
...
for ti = 1:length(Tests)
    ...
    for vi = 1:length(vector)
        ...
        for ep = 1:epochs
            ...
            if ref
                ev_file_name = ['OUTPUT\',Tests(ti).name,'\events_and_labels\ev_',test_data.InitParams.sim_name,'_',num2str(ep),'.txt'];
                simdata_file_name = ['OUTPUT\',Tests(ti).name,'\events_and_labels\simdata_',test_data.InitParams.sim_name,'_',num2str(ep),'.mat'];
            else
